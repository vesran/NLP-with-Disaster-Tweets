...
input_tweet = tf.keras.layers.Input(shape=())
embeddings_layer = tf.keras.layers.Embedding(input_dim=len(vocab)+1, output_dim=100, input_length=max_length)
bilstm_layer = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(128))
